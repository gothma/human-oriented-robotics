...
% Script that runs and evaluates the k-NN classifier using K-fold
...
% ----- k-NN parameters ranges
kvec = [1 5 15];
distmethods = {'l1norm', 'l2norm','linfnorm'};  % Options: 'l1norm', 'l2norm', 'linfnorm'

% ----- Write preamble
disp('k-Nearest Neighbor CV run:');
disp('--------------------------');
...
kind=1;
for k = kvec,
  for idist = 1:length(distmethods),
    ...
    for i = 1:K,
      ...
      disp('Results');
      disp(['- k = ',int2str(k)]);
      disp(['- distmethod: ',distmethods{idist}]);
